...
function setupCommon() {
    document.addEventListener('keydown', function (e) {
        browser.storage.local.get(["shortcutKey"]).then((result) => {
...
function importTabs() {
    validTabs.forEach(tabLink => {
        browser.tabs.create({ url: tabLink.url });
    });
}
...
function setupShortcutKey() {
    browser.storage.local.get(["shortcutKey"]).then((result) => {
        ...
        if (result != undefined && result.shortcutKey != undefined && result.shortcutKey != '') {
            ...
            browser.storage.local.set({ shortcutKey: shortcutKey })
        }
...
function updateShortcutPressHandler(e) {
    ...
    browser.storage.local.set({ shortcutKey: charKey })
